function [synapticMatrix_retinaLGN,percent_node] = LGN_learn_receptiveFields_afterDamage(numLGN, outerRadius, retinaParams, LGN_pos3d, eta, decay_rt, mu_wts, synapticMatrix_retinaLGN, numRetina, pixelKill, sortIdx)
...
retinaParams(1).v(pixelKill) = 0;
nodes_lifeStatus = zeros(totNeurons_Retina,3);
nodes_lifeStatus(pixelKill,:) = repmat([1,0,1],length(pixelKill),1);


% Find all LGN that have receptive fields on the dead pixels
s2Matrix= synapticMatrix_retinaLGN;
s2Matrix(s2Matrix<0.1) = NaN;
s2Matrix = ~isnan(s2Matrix);      

temp1 = find(sum(s2Matrix)<200);

LGN_rf_deadpixel = [];

for j = 1:min(20,length(temp1))
    
    connectLGN = find(s2Matrix(:,temp1(j))>0.1);
    intersectVal = intersect(connectLGN, pixelKill);
    if length(intersectVal) ~= 0
        LGN_rf_deadpixel = [LGN_rf_deadpixel, temp1(j)];
    end
end
...
count_LGN_deadpixel = 0;
for j = 1:numLGN

    connectLGN = find(s2Matrix(:,j)>0.1);
    intersectVal = intersect(connectLGN, pixelKill);
    if length(intersectVal) ~= 0
        count_LGN_deadpixel = count_LGN_deadpixel + 1;
    end
end
...
while(1)
    ...
    if (mod(t,1e5) == 0)
        terminate_choice = input('Do you want to terminate?');
        switch terminate_choice
            case 0
                disp('Continue')
            case 1 
                disp('Terminating'); 
        ...
        hold on
        scatter3(retinaParams.x(pixelKill,2), retinaParams.x(pixelKill,1), zeros(length(pixelKill),1),'m','filled')
        hold on
        ...
        set(gca, 'Visible','off')
        saveas(gca,strcat('E:\GURU_DATA\Project 1\faultTolerance2\',sprintf('damaged_nodes_%d.png',t)));
        clf
...
%                     hold on
%                     scatter(retinaParams(1).x(:,2),retinaParams(1).x(:,1),[],nodes_lifeStatus,'filled')
...
%         end
    ...
    if count_LGN_deadpixel ==0 %or(t==1, (mod(t,1e4) == 0))
        ...
        if length(temp1)>0.9*numLGN 
        
%         figure;
%         ctr = 1;
%         for j = 1:min(20,length(LGN_rf_deadpixel))
%             subplot(4,5,j)
%             clear l
% 
%             l = find(synapticMatrix_retinaLGN(:,LGN_rf_deadpixel(j))>0.1);
%             hold on
%             scatter(retinaParams(1).x(:,2),retinaParams(1).x(:,1),[],nodes_lifeStatus,'filled')
%             scatter(retinaParams(1).x(l,2),retinaParams(1).x(l,1),[],'r','filled')
%             hold on
%             title(sprintf('LGN %d',LGN_rf_deadpixel(j)))
%             ctr = ctr + 1;
%         end   
%         pause(0.2)
        ...
        figure;
        ctr = 1;
        for j = 1:16%min(20,length(LGN_rf_deadpixel))
            subplot(4,4,j)
            clear l
    ...
    for i = 1:numRetina
        ...
        retinaParams(i).v = retinaParams(i).v + 0.5*(0.04*retinaParams(i).v.^2 + 5*retinaParams(i).v + 140 - retinaParams(i).u + retinaParams(i).I);
        retinaParams(i).v(pixelKill) = 0;  % Making membrane potential of dead pixels = 0
        
        retinaParams(i).u = retinaParams(i).u + retinaParams(i).a.*(retinaParams(i).b.*retinaParams(i).v - retinaParams(i).u); 
        retinaParams(i).u(pixelKill) = 0;  % Making membrane potential of dead pixels = 0
