% Developmental algorithm to form a pooling architecture on arbitrary geometry
...
figure; 
scatter(retinaParams_old(1).x(:,2),retinaParams_old(1).x(:,1),'filled')
hold on; k1 = boundary(sketch_boundary1(:,2),sketch_boundary1(:,1));
plot(sketch_boundary1(k1,1),sketch_boundary1(k1,2))

k2 = boundary(sketch_boundary2(:,2),sketch_boundary2(:,1));
plot(sketch_boundary2(k2,1),sketch_boundary2(k2,2))

% Find all coordinates within boundary
[in1, on1] = inpolygon(retinaParams_old(1).x(:,2),retinaParams_old(1).x(:,1),sketch_boundary1(k1,1),sketch_boundary1(k1,2));
[in2, on2] = inpolygon(retinaParams_old(1).x(:,2),retinaParams_old(1).x(:,1),sketch_boundary2(k2,1),sketch_boundary2(k2,2));

ind_withinGeometry = intersect(union(find(in1==1),find(on1==1)),find(in2==0));
...
% Redefine all parameters of the retina.
